...
export function KpiCards() {
  ...
  const pleParams: FindingsQuery = {
    ...buildFindingsQuery({ activeTopicId: "tempdb_memory", selectedClusterId, filters: {}, from, to }, 0, 10),
  ...
  const { data: pleData, isLoading: l4, isFetching: f4 } = useQuery({ queryKey: qk.findings(pleParams), queryFn: () => apiGet<FindingsResponse>("/api/findings", pleParams), staleTime: 30_000, placeholderData: (prev) => prev });
  ...
  const pleFinding = (pleData?.items ?? []).find((finding) => hasNumeric(finding.metrics?.ple_sec));
  const pleSec = toNumber(pleFinding?.metrics?.ple_sec);
  const pleSeverity = pleFinding?.severity;
  const pleColor =
    pleSeverity === "CRITICAL" ? "var(--color-critical)"
    : pleSeverity === "WARNING" ? "var(--color-warning)"
    : "var(--color-success)";
  const pleLabel = pleSec === null ? "No signal" : `${Math.round(pleSec).toLocaleString()}s`;

  const loading = l0 || l1 || l2 || l3 || l4;
  const refreshing = (f0 || f1 || f2 || f3 || f4) && !loading;
  const maxCount = Math.max(1, ...Object.values(counts));

  return (
    <div className="relative grid grid-cols-2 gap-2 lg:grid-cols-4 xl:grid-cols-5">
      ...
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg px-3 py-2.5 flex flex-col gap-1.5 transition-all duration-200 hover:border-[var(--color-border-2)]">
        <div className="flex items-center justify-between gap-3">
          <p className="text-[10px] font-semibold text-[var(--color-muted)] uppercase tracking-[0.08em]">
            Buffer Health
          </p>
          ...
          {loading ? (
            ...
            <p className="text-[22px] font-bold tabular leading-none" style={{ color: pleColor }}>
              {pleLabel}
            </p>
          )}
        </div>
        ...
        <div className="h-1 rounded-full bg-[var(--color-border)] overflow-hidden">
          <div
            className="h-full rounded-full transition-all duration-700 ease-out"
            style={{
              width: loading ? "0%" : pleSec === null ? "18%" : "100%",
              background: pleColor,
            }}
          />
        </div>
