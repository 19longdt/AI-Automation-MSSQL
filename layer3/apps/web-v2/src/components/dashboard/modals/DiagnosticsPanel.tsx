...
const PHASE_GROUPS = [
  { label: "Phase 1 - DMV Snapshot", tools: ["get_blocking_chain", "get_blocked_victims_snapshot", "get_wait_stats", "get_memory_grant", "get_ple_numa", "get_tempdb_usage", "get_ag_status", "get_memory_pressure", "get_resource_governor_stats", "get_cdc_status", "get_missing_indexes", "get_query_stats", "get_query_store_history"] },
  ...
  return (
    <div className="relative space-y-4">
      ...
      {PHASE_GROUPS.map((group) => {
        const inPhase = requested.filter((toolId) => group.tools.includes(toolId));
        if (!inPhase.length) return null;

        return (
          <div key={group.label}>
            <p className="text-[11px] font-semibold text-[var(--color-muted)] uppercase tracking-wide mb-2">{group.label}</p>
            <div className="flex flex-wrap gap-1.5">
              {inPhase.map((toolId) => {
                const result = results[toolId];
                if (!result) return null;

                const cls = STATUS_CLS[result.status] ?? STATUS_CLS.error;
                return (
                  <span key={toolId} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-medium border ${cls}`}>
                    {toolId.replace(/^get_/, "").replace(/_/g, " ")}
                    {result.status === "ok" && result.row_count ? <span className="opacity-70">({result.row_count})</span> : null}
                  </span>
                );
              })}
            </div>
