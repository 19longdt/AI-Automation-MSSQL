...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-8 items-center justify-between gap-1.5 rounded-md border border-[var(--color-border-2)]",
      "bg-[var(--color-surface)] px-2.5 py-1.5 text-[13px] text-[var(--color-text)]",
  ...
  >
    ...
    <SelectPrimitive.Icon asChild>
      <ChevronDown className="h-3.5 w-3.5 text-[var(--color-muted)] shrink-0" />
  ...
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      className={cn(
        ...
        "shadow-lg text-[13px] text-[var(--color-text)]",
        "data-[state=open]:animate-[fade-in_120ms_ease]",
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ...
    className={cn(
      ...
      "data-[highlighted]:bg-[var(--color-primary-soft)] data-[highlighted]:text-[var(--color-primary)]",
      "focus:outline-none",
      className,
    )}
    {...props}
