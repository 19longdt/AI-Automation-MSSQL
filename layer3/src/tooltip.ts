...
function showTooltip(node: Element, tooltip: HTMLElement) {
    ...
    let positionX = cursorX + 12;
    let positionY = cursorY + 12;
    let tipRect = currentTooltip.getBoundingClientRect();
    let maxX = window.innerWidth - tipRect.width - 10;
    let maxY = window.innerHeight - tipRect.height - 10;

    if (positionX > maxX) positionX = Math.max(10, maxX);
    if (positionY > maxY) positionY = Math.max(10, maxY);
    if (positionX < 10) positionX = 10;
    if (positionY < 10) positionY = 10;

    currentTooltip.style.left = positionX + "px";
