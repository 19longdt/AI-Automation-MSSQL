...
function drawLines(container: Element) {
    let diagramCanvases = container.querySelectorAll(".qp-diagram-canvas");
    if (diagramCanvases.length > 0) {
        for (let i = 0; i < diagramCanvases.length; i++) {
            drawLinesForRoot(<HTMLElement>diagramCanvases[i]);
        }
        return;
    }
    ...
    if (root != null) {
        drawLinesForRoot(root);
    }
}

function drawLinesForRoot(root: HTMLElement) {
    let draw = SVG(root);
