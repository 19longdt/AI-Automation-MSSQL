...
**Skill YAML (ví dụ `slow_sessions.yaml`):**
```yaml
skill_id: slow_sessions_v1
issue_types:
  - slow_sessions
...
|---|---|---|
| `get_query_stats` | `dm_exec_query_stats` (incl. spills, group by plan_handle) | slow_sessions, plan_xml |
| `get_query_plan` | `dm_exec_cached_plans` | *(bỏ — finding đã có query_plan_xml)* |
| `get_query_store_history` | `query_store_query`, `query_store_plan`, `query_store_runtime_stats` | slow_sessions, plan_xml |
| `get_statistics_info` | `sys.stats`, `dm_db_stats_properties` | slow_sessions, plan_xml |
| `get_memory_grant` | `dm_exec_query_memory_grants` | slow_sessions, memory |
| `get_blocking_chain` | `dm_exec_requests`, `dm_os_waiting_tasks` | blocking, deadlock |
| `get_wait_stats` | `dm_os_wait_stats` | slow_sessions, wait |
| `get_index_usage` | `dm_db_index_usage_stats` | slow_sessions, plan_xml, index |
| `get_missing_indexes` | `dm_db_missing_index_details` | slow_sessions, plan_xml, index |
...
| `get_resource_governor_stats` | `dm_resource_governor_resource_pools` | slow_sessions, resource |
...
### Ví dụ follow-up sau slow_sessions analysis
...
TelegramBot nhận /analyze
  ...
  → Finding {
      issue_type: "slow_sessions", severity: "CRITICAL",
...
```
SkillLoader.get("slow_sessions") → slow_sessions.yaml
...
Telegram reply:
  🔍 Phân tích: slow_sessions | SQL-NODE-01
  ━━━━━━━━━━━━━━━━━━━━━━
  [analysis_text]
...
1. `slow_sessions` end-to-end: `/analyze <id>` → Telegram reply với analysis + tool_calls_count
